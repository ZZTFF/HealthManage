...
public class MessageController {
     ...
     */
    @GetMapping(value = "/types")//MessageTypeVO属性，type，detail
    public Result<List<MessageTypeVO>> all() {
        MessageType[] messageTypes = MessageType.values();
        List<MessageTypeVO> messageTypeVOS = new ArrayList<>();
        for (MessageType messageType : messageTypes) {//定义循环变量messageType
            MessageTypeVO messageTypeVO = new MessageTypeVO(messageType.getType(), messageType.getDetail());
            messageTypeVOS.add(messageTypeVO);//将MessageType里定义的type和detail给到messageTypeVOS列表
        }
        return ApiResult.success(messageTypeVOS);//由于不需要进行数据库交互，不需要调用service
    }
